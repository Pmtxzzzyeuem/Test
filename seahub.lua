...
local oldcolor = {
    ...
    ["Logo Image"] = "rbxassetid://17510500831",
...
function SaveCustomUISettings()
    local j = game:GetService("HttpService")
    if not isfolder("Windy Hub") then
        makefolder("Windy Hub")
    end
    writefile("Windy Hub/" .. k, j:JSONEncode(CorrectTable(e)))
end
function ReadCustomUISetting()
    local l, m =
        ...
        function()
            ...
            local n = j:JSONDecode(readfile("Windy Hub/" .. k))
...
u.NotiGui.Name = "Windy Hub Notification"
...
-- if not isfolder("Heiskso Hub") then
--     makefolder("Heiskso Hub")
-- end
-- local s,Translated = pcall(function() 
--     return game.HttpService:JSONDecode(readfile("Heiskso Hub/!UIText.json"))
-- end)
-- function SaveFile() 
--     pcall(function() 
--         writefile("Heiskso Hub/!UIText.json",game.HttpService:JSONEncode(getgenv().TranslateCache))
--     end)
-- end
...
function t.CreateNoti(H)
    ...
    local g = V .. "," .. W .. "," .. X
    Q.Text = '<font color="rgb(' .. g .. ')">Windy Hub</font> ' .. getgenv().TitleNameNoti
    table.insert(
        c["Title Text Color"],
        function()
            local V = tostring(u.Getcolor(getgenv().UIColor["Title Text Color"])[1])
    ...
    S.Image = "rbxassetid://17596666700"
...
function t.CreateMain(H)
    local Z = tostring(H.Title) or "さま┊Windy•Hub"
    ...
    ImageButton.Image = "rbxassetid://17510500831"
    ...
    function u.ReloadMain(v)
        ...
        local X = tostring(u.Getcolor(getgenv().UIColor["Title Text Color"])[3])
        local g = V .. "," .. W .. "," .. X
        ad.Text = '<font color="rgb(' .. g .. ')">Windy Hub</font> ' .. getgenv().MainDesc
        table.insert(
            c["Title Text Color"],
            function()
                aa.ImageColor3 = getgenv().UIColor["Title Text Color"]
